...
from ..utils.admin_site_utils import (MeOrAllFilter, is_null_filter_factory, USER_SEARCH_FIELDS)


@admin.register(Notification)
class NotificationAdmin(admin.ModelAdmin):
    list_display = ['user_target', 'status',
                    'scheduled_delivery', 'attempted_delivery']
    list_filter = [
        'status',
    ...
    search_fields = ["user_target__" + field for field in USER_SEARCH_FIELDS] \
                    + ['title', 'body']
...
class OptOutAdmin(admin.ModelAdmin):
    list_display = ['user', 'has_opted_out', 'opt_out_link']
...
class UserTargetAdmin(admin.ModelAdmin):
    ...
    search_fields = USER_SEARCH_FIELDS + ('user_target_id','user_target_friendly_name')
